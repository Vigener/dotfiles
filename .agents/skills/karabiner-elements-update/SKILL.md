...
description: >-
  dotfilesのKarabiner-Elements設定（主にアプリ起動やキーマッピング等）を更新し、READMEを同期して、変更をコミット＆プッシュするスキル。
...
更新後はビルドを実行し、人間向けの `README.md` を同期し、最後に `git-semantic-commit` スキルを持つサブエージェントを呼び出して Git プッシュまで完結させる。
...
### 5. サブエージェントによる自動コミット＆プッシュ
ビルドが成功したら、すべての変更を Git にコミットしてプッシュする。
この操作はトークン節約のため、低コストモデルを使った**サブエージェント**に委譲する。

`invoke_subagent` ツールを使用して、以下の設定でサブエージェントを起動する。

- **Role**: `Git Committer`
- **Prompt**:
  ```
  あなたはGit操作を担当するサブエージェントです。
  現在ワークスペースには Karabiner-Elements の設定変更が含まれています。
  `git-semantic-commit` スキル（~/.gemini/config/skills/git-semantic-commit/SKILL.md）を読み込み、
  そのスキルの手順に従って変更を意味的なグループに分け、コミットし、リモートにプッシュしてください。
  プッシュが完了したら、コミット内容の要約を私（親エージェント）に報告してください。
  ```

※サブエージェントから完了の報告を受け取ったら、ユーザーにもその結果を伝えてタスク完了とする。

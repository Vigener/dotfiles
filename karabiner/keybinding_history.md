...
当初、誤爆緩和策として `cmd+shift+w=text:\x00x`（Shift 必須化）を試用したが、以下の課題が実機で浮き彫りとなった：
1. **キーリピート超高速化による連鎖消滅**: ディレイ／インターバルを詰めている環境下で複数修飾キーを押すと、離脱の僅かな遅れが長押し判定され、2タブ（2ペイン）一気に閉じてしまう事故が発生。
2. **近接誤爆リスク**: `英数+Ctrl+W` 等の代替案も、デタッチ（`英数+Ctrl+Q`）と隣接しているため安全弁にならない。
3. **不可逆操作に対する設計思想**: tmux / herdr 等のターミナルマルチプレクサにおいて、可逆操作（移動・分割・新規タブ等）は直バインドで加速しても、プロセスを道連れにする不可逆操作（kill）だけは直バインドを避け、二段構えの prefix（またはシェルの `exit` / `Ctrl+D`）に委ねるのが鉄則。

結論として、Ghostty 側で `cmd+w=unbind`, `cmd+shift+w=unbind` とし、直バインドによるペイン閉じを全廃。左手片手で軽快かつ誤爆ゼロで打てる由緒ある prefix **`Ctrl+Space` → `x`** のみに一本化することとした。
...
6. **英数+Ctrl+Q** は Ghostty のみ Ctrl+Q を通し、Ghostty が prefix+q（デタッチ）へ。英数+Q は Esc、Cmd+Q はアプリ終了のまま。
